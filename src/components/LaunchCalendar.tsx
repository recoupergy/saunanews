...
const typeColors: Record<string, string> = {
  ...
  Report: 'bg-slate/10 text-slate',
};

export default function LaunchCalendar() {
  return (
    <div className="bg-ivory border border-border rounded-xl overflow-hidden">
      <div className="px-5 py-4 border-b border-border flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-charcoal">Launch Calendar</h3>
        </div>
      ...
      <div className="divide-y divide-border">
        {upcomingLaunches.map((item, i) => {
          const inner = (
            <>
              <span className="text-xs font-semibold text-stone-dark w-12 shrink-0 tabular-nums">
                {item.date}
              </span>
              <div className="flex-1 min-w-0">
                <span className="text-sm font-medium text-charcoal group-hover:text-green transition-colors block truncate">
                  {item.title}
                </span>
                <span className="text-xs text-stone-dark">{item.brand}</span>
              </div>
          ...
          const cls = "flex items-center gap-4 px-5 py-3.5 hover:bg-cream transition-colors group";
          return item.href ? (
